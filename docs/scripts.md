...
| `fm-task-number-lib.sh`  | One owner of the captain's project numbers: the board bases, the reserved ship number, the 100 pool and its hold window, and the `fm-<number>-<id>` endpoint label every surface derives |
| `fm-dashboard.mjs`       | Generate one read-only dashboard board: the git health of that board's projects beside one row per running task on them, written as one static HTML file with no server and no path back to the fleet. `--group` selects the board (`/dashboard` and `/dashboard-personal` are the same generator); `--watch` keeps re-writing it until stopped |
| `fm-dashboard-open.sh`   | The captain's `dashboard` command: generate one board, open it, and leave exactly one detached refresh loop running for it, with `--stop`, `--status`, and a pinned default interval. Identifies its own loop by recorded pid plus that pid's own process identity, reports a loop it did not start instead of adding a second, and decides both from one pid's own argv words rather than from searched `ps` text |
| `fm-install-dashboard-command.sh` | Link this home's `dashboard` command into the captain's own bin directory, refusing to replace a `dashboard` this home did not install and reporting a directory his PATH does not reach; `--check` reports without changing anything |
