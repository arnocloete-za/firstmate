...
| `fm-dashboard-server.py` | Serve the built `/dashboard` board on loopback and own the loopback `POST /run` contract that launches a registered project's run script in the `dashboard` tmux session |
