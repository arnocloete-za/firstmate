...
A task whose `state/<id>.conn` flag is held is stood off entirely for that poll - no steering-inbox re-ring, no stale or wedge escalation - because the captain is working in that worker's own terminal and is its second supervisor while he is there.
That stand-off defers rather than drops: the flag expires on a bounded idle window refreshed by each captain message, the idle timer and escalation count restart so the wedge clock is measured from his departure, and status appends are untouched so what he settled still surfaces through the ordinary signal path ([`bin/fm-conn-lib.sh`](../bin/fm-conn-lib.sh) owns the flag).
...
The default branch is resolved per project by `fm_default_branch()` in `bin/fm-tangle-lib.sh`, the same single resolver `bin/fm-dashboard.mjs` uses, since registered projects sit on `master`, `main`, `develop`, and `test` alike.
