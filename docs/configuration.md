...
`.dashboard/` holds the `/dashboard` command's generated, regenerate-on-each-run status page; `bin/fm-dashboard.mjs`'s header owns its exact contents. Nothing serves it - it is a static file the captain opens.
