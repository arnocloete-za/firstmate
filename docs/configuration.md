...
`.dashboard/` holds the generated, regenerate-on-each-run status page of each dashboard board - `/dashboard` and `/dashboard-personal` each write their own page there - and, while a `--watch` run is keeping one of those pages current, the one sibling file that page reads to notice it changed; `bin/fm-dashboard.mjs`'s header owns the board table, each board's page name, and the watch contract. Nothing serves any of these files - they are static files the captain opens.
...
Secondmate routes cover `no-mistakes` and `direct-PR` projects; `local-only` and `project-branch` projects remain main-firstmate work.
...
Normal completed runs keep local-only, no-origin, and project-branch-on-its-batch-branch skips silent.
...
FM_CONN_IDLE_SECS=900              # idle seconds a state/<id>.conn record stays held before it lapses and that task returns to ordinary supervision; each captain message in the worker's own terminal refreshes it, and expiry is mandatory so a terminal he walked away from cannot stay unsupervised (bin/fm-conn-lib.sh)
