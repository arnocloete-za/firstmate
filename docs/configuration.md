...
`.dashboard/` holds the `/dashboard` command's generated, regenerate-on-each-run status page, and, while a `--watch` run is keeping that page current, the one sibling file the page reads to notice it changed; `bin/fm-dashboard.mjs`'s header owns its exact contents and the watch contract. Nothing serves either file - they are static files the captain opens.
