...
`.dashboard/` holds the `/dashboard` command's generated, regenerate-on-each-run status page, and is gitignored so regenerating it never shows up as uncommitted work in a home that is itself a checkout; `bin/fm-dashboard-serve.sh`'s header owns its exact contents and server lifecycle.
...
`dirty_status` in [`bin/fm-ff-lib.sh`](../bin/fm-ff-lib.sh) additionally excludes exactly two Firstmate-generated untracked paths from its dirtiness verdict: the `.fm-secondmate-home` marker, for the callers that opt in, and the `/dashboard` output directory `.dashboard/`, always.
