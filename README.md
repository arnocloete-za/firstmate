...
| `/dashboard`       | Run bearings, then build and serve a local visual project-status board showing every registered project's cleanliness, last commit, and branch, with a per-project Run control that launches that project's registered run script in a local tmux session |
