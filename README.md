...
| `/dashboard`       | Run bearings, then build and serve one local visual board: the work running right now - what each task is doing, what it waits on, and the copyable command for its own terminal, ranked so whatever wants the captain is first - beside every registered project's cleanliness, last commit, and branch |
