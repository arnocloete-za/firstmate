...
- An Underway row's `num` is the captain's own number for that work, not an internal id, so it is his to read under `AGENTS.md` section 9 rather than routing-only payload.
